...
const driver = ManagerDrivers.getDriver('energylink');
		...
		refreshIntervalId = setInterval(function () {
			...
			Object.keys(devices).forEach(function (device_id) {
				...
				if(devices[device_id].settings.homewizard_id !== undefined ) {
					// console.log('Poll for '+devices[device_id].getName());
							...
							if (Object.keys(callback).length > 0) {
								try {
									...
									try {
										...
										// Consumed gas
										devices[device_id].setCapabilityValue("meter_gas.today", gas_daytotal_cons);
									}
									catch(err) {
									...
									devices[device_id].setCapabilityValue('measure_power.used', energy_current_cons);
									// Consumed elec current
									devices[device_id].setCapabilityValue('measure_power', energy_current_netto);
									// Consumed elec current Netto
									devices[device_id].setCapabilityValue('measure_power.netto', energy_current_netto);
									// Consumed elec total day
									devices[device_id].setCapabilityValue('meter_power.used', energy_daytotal_cons);
									// Consumed elec total day
									devices[device_id].setCapabilityValue('meter_power.aggr', energy_daytotal_aggr);
									...
									if(value_s1 == 'solar' || value_s2 == 'solar') {
										devices[device_id].setCapabilityValue("measure_power.s1", solar_current_prod );
										devices[device_id].setCapabilityValue("meter_power.s1", solar_daytotal_prod );
									}
									...
									if (value_s1 == 'water' ) {
										...
										// Used water m3
										devices[device_id].setCapabilityValue("meter_water", water_daytotal_cons);
										devices[device_id].setCapabilityValue("measure_water", water_current_cons);
									...
									// Trigger flows
				                    if (energy_current_cons != devices[device_id].getStoreValue("last_measure_power_used") && energy_current_cons != undefined && energy_current_cons != null) {
				                        console.log("Current Power - "+ energy_current_cons);
										me.flowTriggerPowerUsed(me, { power_used: energy_current_cons });
										devices[device_id].setStoreValue("last_measure_power_used",energy_current_cons);
				                    }
			...
			Object.keys(devices).forEach(function (device_id) {
				if (devices[device_id].settings.homewizard_id !== undefined) {
					...
					homewizard.getDeviceData(homewizard_id, 'energylink_el', function (callback) {
						...
						if (Object.keys(callback).length > 0) {
							try {
								devices[device_id].setAvailable();
								...
								// Save export data
								devices[device_id].setCapabilityValue("meter_gas.reading", metered_gas);
								devices[device_id].setCapabilityValue("meter_power.consumed.t1", metered_electricity_consumed_t1);
								devices[device_id].setCapabilityValue("meter_power.produced.t1", metered_electricity_produced_t1);
								devices[device_id].setCapabilityValue("meter_power.consumed.t2", metered_electricity_consumed_t2);
								devices[device_id].setCapabilityValue("meter_power.produced.t2", metered_electricity_produced_t2);


							} catch (err) {
								// Error with Energylink no data in Energylink
								console.log("No Energylink found");
								devices[device_id].setUnavailable();
							}
						}
